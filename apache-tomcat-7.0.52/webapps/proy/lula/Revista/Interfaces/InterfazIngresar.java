   ...
   public void doGet(HttpServletRequest request, HttpServletResponse response) throws IOException {
    ...
    if((email == null && pass == null)||(email == "" && pass == "")){ // Si es null significa que no se ha mandado solicitud
      //Encabezado donde sale la navbar dependiendo de que usuario sea
		...
		out.println("				</nav>		");
		out.println("				<H1 align=\"center\">Ingrese a su cuenta! (Terminado)</H1>	");
	...
	//Ingresar ci; /***********************Pendiente**************************************/
    //ci = new Ingresar();/***********************Pendiente**************************************/
	String email = thisRequest.getParameter("email");
	String pass = thisRequest.getParameter("pass");
	//String tipo = ci.accesar(email,pass);/***********************Pendiente**************************************/
    if (true){ /*tipo != null ***********************Pendiente**************************************/
		out.println("<script>window.location.replace('Revista?tipo=s"+"');</script>"); /*tipo************************Pendiente**************************************/
	}
